...
<div class="app">
    ...
    <aside class="sidebar">
        ...
        <nav class="nav" aria-label="Módulos principales">
            ...
            <a href="{{ route('estudiantes.index') }}"><svg class="i"><use href="#i-students"/></svg> Estudiantes</a>
...
<main>
    ...
    <div class="head">
        <div>
            <h1>Nuevo usuario</h1>
            <p>Crea una cuenta de acceso y asigna sus roles.</p>
        </div>
    </div>

    @if ($errors->any())
        <div class="flash" style="background:#fef2f2;color:#dc2626;">
    ...
    <form method="POST" action="{{ route('usuarios.store') }}" class="card" style="padding:24px;display:grid;gap:16px;max-width:520px;">
        @csrf

        <label>Usuario
            <input type="text" name="usuario" value="{{ old('usuario') }}" required
                   style="width:100%;margin-top:4px;padding:10px 12px;border:1px solid var(--line);border-radius:8px;">
        </label>
        ...
        <label>Estado
            <select name="estado" required style="width:100%;margin-top:4px;padding:10px 12px;border:1px solid var(--line);border-radius:8px;">
        ...
        <fieldset style="border:1px solid var(--line);border-radius:8px;padding:12px;">
            <legend style="padding:0 6px;font-size:12px;color:var(--muted);">Roles</legend>
            @foreach ($roles as $rol)
                <label style="display:flex;align-items:center;gap:8px;padding:4px 0;">
                    <input type="checkbox" name="roles[]" value="{{ $rol->id_rol }}"
                           @checked(collect(old('roles'))->contains($rol->id_rol))>
                    {{ $rol->nombre }}
                </label>
            @endforeach
        </fieldset>
        ...
        <div style="display:flex;gap:12px;">
            ...
            <a href="{{ route('usuarios.index') }}" style="align-self:center;color:var(--muted);">Cancelar</a>
        </div>
    </form>
</main>
    </div>
