...
<head>
    ...
    <title>Editar estudiante · SCIEM</title>
    <style>
        :root { --navy-900:#0f1d4d; --blue-700:#1e3a8a; --bg:#f3f5fb;
...
<body>
<main>
    <nav class="crumb"><a href="{{ route('estudiantes.index') }}">Estudiantes</a> › <span>Editar</span></nav>
    ...
    <div class="card">
        <form method="POST" action="{{ route('estudiantes.update', $estudiante) }}">
            ...
            <label>Código universitario
                <input name="codigo_universitario" value="{{ old('codigo_universitario', $estudiante->codigo_universitario) }}">
                @error('codigo_universitario') <span class="error">{{ $message }}</span> @enderror
            </label>

            <label>CI / Documento de identidad
                <input name="documento_identidad" value="{{ old('documento_identidad', $estudiante->documento_identidad) }}">
                @error('documento_identidad') <span class="error">{{ $message }}</span> @enderror
            </label>

            <label>Nombres
                <input name="nombres" value="{{ old('nombres', $estudiante->nombres) }}">
                @error('nombres') <span class="error">{{ $message }}</span> @enderror
            </label>

            <label>Apellidos
                <input name="apellidos" value="{{ old('apellidos', $estudiante->apellidos) }}">
                @error('apellidos') <span class="error">{{ $message }}</span> @enderror
            </label>

            <label>Carrera
                <input name="carrera" value="{{ old('carrera', $estudiante->carrera) }}">
                @error('carrera') <span class="error">{{ $message }}</span> @enderror
            </label>

            <label>Correo institucional
                <input type="email" name="correo_institucional" value="{{ old('correo_institucional', $estudiante->correo_institucional) }}">
            ...
            <label>Estado
                <select name="estado">
                    ...
                    <option value="OBSERVADO" @selected(old('estado', $estudiante->estado) === 'OBSERVADO')>Observado</option>
